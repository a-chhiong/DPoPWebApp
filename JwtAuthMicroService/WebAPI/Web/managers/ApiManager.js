...
import { dpopMgr } from './DPoPManger.js';
        ...
        this.authApi.interceptors.request.use(
            async (config) => {
                try {
                    ...
                    const dpopProof = await dpopMgr.createDPoP(htm, htu, token);
                    config.headers.DPoP = dpopProof;
                    return config;
                } catch (e) {
                    console.warn(`[ApiManager] Request Error: ${e}`);
